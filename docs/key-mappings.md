...
| ActionAddTempoChange   | b + T        | Add tempo change note to the grid. When the playback cursor reaches this note, the playing tempo changes to the note's value; +/- or digit entry sets that value. Does not affect the sequence's start tempo, and is not undoable when triggered by playback                          |
